...
public class SendReq extends MultimediaMessagePdu {
    ...
    public void prepareFromAddress(Context context, String fromAddress, int subscriptionId) {
        String phoneNumber = myPhoneNumberForSubscription(context, subscriptionId);
    ...
    /*
     * Local replacement for klinker's Utils.getMyPhoneNumberFromSubscription:
     * that helper pulled in the whole MMS network stack, which this trimmed
     * module does not carry. The lookup itself is the same - match the
     * subscription, then read its line-1 number.
     */
    private static String myPhoneNumberForSubscription(Context context, int subscriptionId) {
        ...
        for (SubscriptionInfo info : active) {
            if (info.getSubscriptionId() != subscriptionId) continue;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                TelephonyManager forSub = telephony.createForSubscriptionId(subscriptionId);
                return forSub == null ? null : forSub.getLine1Number();
            }
